...
export async function inspectReport(pollutionSourceId: number) {
    return Taro.request({
        url: `/meijing-inspect-server/api/v1/construction-site-manager/patrol/latest`,
        method: 'GET',
        data:{
            pollutionSourceId
        }
...
export async function inspectDetails(Id: number) {
    return Taro.request({
        url: `/meijing-inspect-server/api/v1/construction-site-manager/detail/${Id}`,
        method: 'GET'
    });
}
...
export async function inspectContentSumbit(param:any){
    return Taro.request({
        url: '/meijing-inspect-server/api/v1/construction-site-manager/special-item-patrol',
        method: 'POST',
        data: {
            ...param
        }
...
export async function inspectSumbit(param:any){
    return Taro.request({
        url: '/meijing-inspect-server/api/v1/construction-site-manager/report',
        method: 'POST',
        data: {
            ...param
        }
...
export async function getWeather(location) {
    ...
    return Taro.request({
        url: `/model-data-service/open/api/v1/camera/weather?lat=${lat}&lon=${lon}`,
        method: "GET",
    })
}

// 获取管控建议信息
export async function getControlProposal() {
    return Taro.request({
        url: `/meijing-inspect-server/api/v1/construction-site-manager/control-proposal`,
        method: "GET",
    })
}

// 获取当前考核站点
export async function getDivisionExamineSites(division: string) {
    return Taro.request({
        url: "/iot-data-server/api/v1/division-examine-sites/all?divisionCode=" + division,
        method: "GET",
    })
}