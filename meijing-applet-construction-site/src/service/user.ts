...
export async function viewTenantUserDetails(userId: number) {
    return Taro.request({
        url: `/simple-user-server/api/v2/tenant-user/${userId}/user-detail`,
    });
}
...
export async function getUserInfo(userId: number) {
    return Taro.request({
        url: `/simple-user-server/api/v1/users/${userId}`,
    });
}
 ...
 * @param user 用户信息
 */
export async function updateUser(tenantUser: tenantUser) {
    return Taro.request({
        url: `/simple-user-server/api/v2/tenant-user/${tenantUser.userId}`,
        data: tenantUser,
        method: 'POST'
    });
}
 ...
 */
export async function departmentUsers(tenantCode: string) {
    return Taro.request({
        url: `/simple-user-server/api/v2/departments/${tenantCode}/department-users`,
    });
}
...
export async function getQrCode(fileName: string, scene: string, page: string) {
    return await Taro.request({
        url: `/simple-user-server/api/v1/auth/qr-code`,
        method: 'POST',
        data: {
            fileName: fileName,
            scene: scene,
