...
export async function getAlarmRecipientList(id: number, offset: number = 0, limit: number = 200) {
    return Taro.request({
        url: `/meijing-message-server/api/v1/recipient-message/search-with-source?sourceType=alarm-analysis&sourceId=${id}&offset=${offset}&limit=${limit}`,
    });
}
...
export async function sitesFactors(monitorType: string) {
    return Taro.request({
        url: `/iot-manager-server/api/v1/sites/factors?monitorType=${monitorType}`,
        method: 'GET'
    });
}
 ...
 * 获取站点报警详情
 */
export async function listSites(pollutionCode: string) {
    return Taro.request({
        url: `/meijing-pollution-source-server/api/v1/pollution-source-site/${pollutionCode}/list-sites`,
        method: 'GET'
    });
}
