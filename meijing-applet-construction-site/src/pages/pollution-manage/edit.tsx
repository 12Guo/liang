...
import { getPollutionSourceTypeList, pollutionSourceTypeStatus, getPollutionDetail, editpollutionPources, enumConstructionSiteType, enumConstructionSiteProgress, editpollutionPourcesQuery } from '../../service/pollutionType'
...
class MarkAdd extends Component<AddProps, AddState> {
    ...
    getDepartemtList = async () => {
        try {
            ...
            const res = await listDepartmentByDivision(currentUserDetails.pollutionSourceInfo.divisionCode);
            const { data } = res;
            this.setState({
                departemtList: data,
            })
    ...
    onSubmit = () => {
        ...
        this.setState({
            ...
            if (!dataObj.divisionCode || !dataObj.divisionName) {
                //如果未获取到上报位置所在的行政区
                dataObj.divisionCode = userDetails.pollutionSourceInfo.divisionCode;
                dataObj.divisionName = userDetails.pollutionSourceInfo.divisionName;
            }
            ...
            try {
                const { pollutionSourceId, divisionCode, divisionName } = userDetails.pollutionSourceInfo;
                const query: editpollutionPourcesQuery = {
                    pollutionSourceId, divisionCode, divisionName
                }
                await editpollutionPources(dataObj, query);
                Taro.navigateBack();
            } catch (error) {
                console.log(error);
            }
    ...
    onDepartmentChange() {
        const { userStore: { userDetails: currentUserDetails } } = this.props;
        Taro.navigateTo({
            url: `/pages/department_select/index?dataCode=superviseDepartment&divisionCode=${currentUserDetails.pollutionSourceInfo.divisionCode}`
        });
    }
                        ...
                        <View className='list-item'>
                            ...
                            <View className='right'>
                                <View className='right__container'>
                                    {/* <Picker disabled mode='selector' value={ptIndex} range={pollutionTypeList} range-key='name' onChange={this.onPollutionTypeChange.bind(this)}> */}
                        ...
                        <View className={cn('list-item-more', { 'list-item-more--show': showMore })}>
                            ...
                            <View className='list-item'>
                                ...
                                <View className='right'>
                                    ...
                                    <View className='right__container'>
                                        <Text className='txt'>{get(dataObj, 'superviseDepartmentName', '无') || '无'}</Text>
                                        {/* <AtIcon className='chevron_right' value='chevron-right' size='20' color='#7A8499' /> */}
                                    </View>
                            ...
                            <View className='list-item'>
                                ...
                                <View className='right'>
                                    ...
                                    <View className='right__container'>
                                        <Text className='txt'>{get(dataObj, 'inspectorName', '无') || '无'}</Text>
                                        {/* <AtIcon className='chevron_right' value='chevron-right' size='20' color='#7A8499' /> */}
                                    </View>
                                </View>
                            </View>
                            <View className='list-item'>
                                ...
                                <View className='right'>
                                    ...
                                    <View className='right__container' >
                                        <Text className='txt'>{get(dataObj, 'supervisorName', '无') || '无'}</Text>
                                        {/* <AtIcon className='chevron_right' value='chevron-right' size='20' color='#7A8499' /> */}
                                    </View>
                                </View>
                            </View>
                            <View className='list-item'>
                                ...
                                <View className='right'>
                                    ...
                                    <View className='right__container'>
                                        <Text className='txt'>{get(dataObj, 'leaderName', '无') || '无'}</Text>
                                        {/* <AtIcon className='chevron_right' value='chevron-right' size='20' color='#7A8499' /> */}
                                    </View>
                                </View>
                            </View>
