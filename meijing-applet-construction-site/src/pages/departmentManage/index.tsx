...
import { Department, treeByTenantCode, rootDivisionCode, getNodeById, deleteByTenant } from '../../service/department'
...
export default class DepartmentManage extends Component<DepartmentManageProps, DepartmentManageState> {
  ...
  componentDidShow() {
    this.initTreeByTenantCode();
  }

  async initTreeByTenantCode(){
  ...
  editDepartment = department => {
    Taro.navigateTo({
      url: `./edit?type=edit&id=${department.id || 0}&name=${department.name || ''}`
    })
    this.setState({ selectedDepartment: {} })
  }
  ...
  onConfirm = async () => {
    ...
    try {
      const res = await deleteByTenant(tenantCode, selectedDepartment.id)
        ...
        >
          {
            isEmptyData ? showEmpty :
              <View className='listWrap'>
                {
                  currentList.map((department, index:number) => {
                    return (
                      <View key={department.id} className={cn('list-item', { 'list-item__gray': selectedDepartment.id === department.id })}>
                        ...
                        <View className={cn('list-float', { 'list-float__show': selectedDepartment.id === department.id, 'list-float__bottom': index > 7 })}>
