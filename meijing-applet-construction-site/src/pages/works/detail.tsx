...
import {
  ...
  getViewList, getReplyList, InspectDetail, assign, share, view, praise, supervise, InspectInfoType
...
import { rootSourceBaseUrl, getUserAvatarUrl, baseUrl, rootConstructionSourceBaseUrl } from '@common/utils/requests'
import { eventSource } from "../../common/utils/divisionUtils";  // 事件来源
...
// const eventDetailBgImg = rootConstructionSourceBaseUrl + "/assets/pages/green-construction/event-detail-button-bg.png";
// const eventDetailDispatchImg = rootConstructionSourceBaseUrl + "/assets/pages/green-construction/event-detail-dispatch.png";
// const eventDetailConstructionImg = rootConstructionSourceBaseUrl + "/assets/pages/green-construction/event-detail-construction.png";
...
export default class Detail extends Component<InspectDetailProps, InspectDetailState> {
  ...
  onNaviToDetail = () => {
    ...
    Taro.navigateTo({
      url: `/pages/work_options/inspectList?inspectId=${inspectDetail.id}`
    });
  }
  ...
  render() {
    ...
    const hasSupervisePermission: boolean = isAnyRole(userDetails.roles, [SUPERVISOR, LEADER]);
    ...
    const showAnalysis = inspectDetail && inspectDetail.type === InspectInfoType.INCIDENT && !isEmpty(inspectDetail.impactAnalysisData)
    const showInspectList = inspectDetail && inspectDetail.source === 'MEIJING_CONSTRUCTION_SITE'
    console.log("showAnalysis=", showAnalysis, 'showInspectList=', showInspectList)
    return (
      <View className={`content device_pd_${statusBarHeight}`}>
        ...
        >
...

          {
            ...
            <View className='pollutionType contentItem splitBorderBottom'>
              ...
              <Text className="pollutionSource">来源：{eventSource[inspectDetail.source]}</Text>
          ...
          <View className='contentItem contentItemList'>
            ...
            <View className='propertyItem' onClick={this.onUserHandle.bind(this, 2)}>
              ...
              <Text className='propertyName'>浏览记录 ({viewPageParams.list.length}）</Text>
          ...
          {
            (showAnalysis || showInspectList) &&
            <View className='impactView'>

              <View className="onNaviTo">
                {
                  showAnalysis &&
                ...
                {
                  showAnalysis && showInspectList && <View className="devideLine"></View>
                }
                {
                  showInspectList &&
                  <View className='itemView' onClick={this.onNaviToDetail.bind(this)}>
                    <Image className='icon' src={`${rootConstructionSourceBaseUrl}/assets/pages/work/siteInspect.png`} />
                    <Text className='text'>工地巡查清单</Text>
                  </View>
                }
        ...
        {
          inspectDetail && hasSupervisePermission && (inspectDetail.type !== InspectInfoType.PATROL) &&
