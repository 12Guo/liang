...
class Index extends Component<MyProps, MyState> {
    ...
    onSubmit = async () => {
        ...
        this.setState({
            ...
            if (newFiles && newFiles.length > 0) {
                ...
                if (!isEmpty(dataObj.pictureOssKeys)) {
                    dataObj.pictureOssKeys = dataObj.pictureOssKeys.join(',') + ',' + pictureOssKeys.join(',')
                } else {
    ...
    onImagePickChange = (newFiles, operationType: string, index) => {
        if (operationType === 'remove') {
            ...
            if (pictureOssKeys && pictureOssLinks) {
                pictureOssKeysNew.splice(index, 1)
                pictureOssLinks.splice(index, 1)
            }
            this.setState({
                files: newFiles,
                showUploadBtn: true,
