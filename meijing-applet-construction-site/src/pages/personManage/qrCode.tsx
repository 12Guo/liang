...
import { getQrCode } from '../../service/user';
import './qrcode.scss'
...
export default class QRCode extends Component<QRCodeProps, QRCodeState> {
  ...
  async componentDidMount() {
    ...
    let scene: string = "value=" + userDetails.pollutionSourceInfo.divisionCode + "&type=division_code";
    let page: string = `pages/login/login`
    let qrCodeResponse = await getQrCode(userDetails.pollutionSourceInfo.divisionCode + ".jpg", scene, page);
    this.setState({
    ...
    return (
      <View className="root_view">
        <View className="image_group">
          ...
          <View className="division_name">{userDetails.pollutionSourceInfo.divisionName}</View>
