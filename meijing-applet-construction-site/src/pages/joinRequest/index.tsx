...
export default class JoinRequest extends Component<JoinRequestProps, JoinRequestState> {
  ...
  handleConfirm = () => {
    ...
    this.setState({
      ...
      currentJoinId && this.agree(currentJoinId)
    })
  }
              ...
              UserJoinRequestStatus.CONFIRMING == jRequest.status ?
                (
                  <View className='reject_agree'>
                    <Button plain={true} className='reject_button' onClick={this.reject.bind(this, jRequest.id)} >拒绝</Button>
                    <Button plain={true} className='agree_button' onClick={this.showAgree.bind(this, jRequest.id)}>同意</Button>
                  </View>
                )
                :
                (
