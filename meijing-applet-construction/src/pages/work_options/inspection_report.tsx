...
const arrow_white = `${rootConstructionSourceBaseUrl}/assets/pages/work/arrow_write.png`
...
class Index extends Component<MyProps, MyState> {
    ...
    isHaveCache() {
        ...
        if (Object.keys(cache).length !== 0 || getCurrentPage().data.SiteData) {
            this.setState({
                pollutionSourceId: cache.pollutionSourceId || 0,
                constructionSiteProgress: cache.constructionSiteProgress || '',
                pollutionSourceStatusId: cache.pollutionSourceStatusId || 1,
    ...
    getResquestData = async () => {
        let { isOpen, siteName, pollutionSourceId, constructionSiteProgress, progressStatusList, progressStatus, currentWorkStatus } = this.state
        let currentPage = getCurrentPage().data.SiteData;
        let progressStatusTemp;
        if (currentPage && currentPage.appendDatas) {
            progressStatusList.forEach(element => {
                if (element.value === currentPage.appendDatas.constructionSiteProgress) {
        ...
        this.setState({
            siteName: currentPage ? currentPage.name : siteName,
            pollutionSourceId: currentPage ? currentPage.id : pollutionSourceId,
            constructionSiteProgress: currentPage && currentPage.appendDatas ? currentPage.appendDatas.constructionSiteProgress : constructionSiteProgress,
            pollutionSourceStatusId: currentPage ? currentPage.statusId : pollutionSourceId,
            progressStatus: currentPage ? progressStatusTemp : progressStatus,
            currentWorkStatus: currentPage ? currentPage.status : currentWorkStatus,
        }, async () => {
            if (currentPage) {
                // 如果切换污染源或者调用，则
                Taro.setStorageSync('inspection_report_cache', {})
    ...
    // 获取经纬度并且刷新
    getInitData = async () => {
        let { isOpen,progressStatusList} = this.state
        let update: InfoType[] = []
        let update_n: InfoType[] = []
        ...
        let ResponseId = await getSiteId(location)

        let currentSite = ResponseId.data.entries[0]
        let progressStatusTemp;
        if (currentSite && currentSite.appendDatas) {
            progressStatusList.forEach(element => {
                if (element.value === currentSite.appendDatas.constructionSiteProgress) {
                    progressStatusTemp = element.label
                }
            });
        }
        this.setState({
            pollutionSourceId: ResponseId.data.entries[0].id,
            siteName: ResponseId.data.entries[0].name,
            currentWorkStatus: ResponseId.data.entries[0].status,
            progressStatus: progressStatusTemp,
            pollutionSourceStatusId: ResponseId.data.entries[0].statusId,
            constructionSiteProgress: get(ResponseId,'data.entries[0].appendDatas.constructionSiteProgress')

        })

        const resp = await inspectReport(ResponseId.data.entries[0].id)
    ...
    onNaviSelectSite = () => {
        ...
        Taro.navigateTo({
            url: '/pages/work_options/selectSite'
        });
    }
    ...
    render() {
        ...
        const { submitButtonLoading, isAuthSetting, isOpen, update_n, update, inspectReportData, siteName, progressStatusList, progressStatus, currentWorkStatus, currentWorkStatusList, isSubmitFlag, isChangeSite } = this.state

        return (
            <View className="report">

                <TopBar img ={arrow_white} title='巡查上报' onBack={this.onBackHandle} background={'#107EFF'} color={'#fff'} />
                ...
                <AtModal isOpened={isChangeSite}>
                    ...
                    <AtModalAction> <Button onClick={() => { this.setState({ isChangeSite: false }) }}>取消</Button> <Button onClick={this.onConfirmChange.bind(this)}>确定</Button> </AtModalAction>
                </AtModal>
            </View>
        );
    }
