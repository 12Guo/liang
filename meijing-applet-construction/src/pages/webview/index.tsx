  ...
  bindmessage(data: any) {
    if (data && data.detail && data.detail.data) {
      if (data.detail.data[0].type) {
        const dataType = get(data, 'detail.data[0].dataType', 'all') || 'all'
        Taro.navigateTo({
          url: `/pages/webview/horizontal_view?url=${encodeURIComponent(data.detail.data[0].url)}&from=${data.detail.data[0].from}&dataType=${dataType}`
        })
      }
      else {
