...
export default class Filter extends Component<FilterProps, FilterState> {
    ...
    constructor(props) {
        ...
        const workSearchFilter = Taro.getStorageSync('problem-search-filter')
        this.state = {
            configList: [
                {
                    name: 'order', title: '排序方式', items: [
                        { label: '问题数最高', value: 'PROBLEM_NUM_DESC' },
                        { label: '问题数最低', value: 'PROBLEM_NUM_ASC' },
                        { label: '处置率最高', value: 'DISPOSAL_RATE_DESC' },
                        { label: '处置率最低', value: 'DISPOSAL_RATE_ASC' },
                    ]
                },
                {
                    name: 'departmentIds', title: '部门', items: []
                }
            ],
    ...
    saveBack = () => {
        ...
        Taro.setStorageSync('problem-search-filter', { ...configChecked, startTime, endTime })
        Taro.navigateBack()
    }
    ...
    onReset = () => {
        Taro.setStorageSync('problem-search-filter', {})
        const { oriStartTime: startTime, oriEndTime: endTime } = this.$router.params
        this.setState({
