...
class TaskDispatchMessage extends Taro.Component {
  ...
  getMoreMessages(callback?:any) {
    const { limit, offset , hasMore, messageList } = this.state;
    if (!hasMore) { return; }

    let newOffset = offset + limit;

    getMessageList(newOffset, limit)
        .then((resp) => {
            const { data: { entries } } = resp;
            this.setState({
  ...
  render() {
    ...
    hasMore = messageListData.length != 0;
