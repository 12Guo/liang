...
import { FactorCode, getFactorName, listDataByTimeNew, alarmsList, getSiteFactorConfigBySiteType, alarmsListByPage, sprayHistories } from '../../service/alarm'
import './detail.scss'
...
export default class Status extends Component<DetailProps, DetailState> {
  ...
  componentWillMount() {
    const { siteDetail } = this.state
    Taro.setNavigationBarTitle({
      title: siteDetail.pollutionSourceName
    })
  }
  ...
  componentDidMount() {
    try {
      const { siteDetail } = this.state
      Promise.all([getSiteFactorConfigBySiteType('construction_dust')]).then(([factorRes]) => {
        const siteInfos = get(siteDetail, 'siteInfos', []);
        const factorsList = get(factorRes, 'data', []).map(factor => ({ label: factor.name, value: factor.code }))
        this.setState({
          factors: factorsList,
  ...
  //渲染折线图
  showLineCharts = () => {
    const { siteHourDataList, factorCode, circleList, tabId } = this.state;
    let xAxisData: any[] = [];
    let seriesData: any[] = [];
