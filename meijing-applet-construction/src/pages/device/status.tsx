...
import { constructionSiteVideoStatusList } from '../../service/home'
import EmptyHolder from '@common/components/EmptyHolder'
import DeviceItem from '@common/components/FbiItems/DeviceItem'
...
export default class Status extends Component<StatusProps, StatusState> {
  ...
  componentDidMount() {
    constructionSiteVideoStatusList().then((videoStatusListRes) => {
      this.setState({
    ...
    return (
      <View className='status-page'>
        ...
        <View className='body'>
          {
            //@ts-ignore
            list.map((item, index) => <DeviceItem key={item.pollutionSourceId} data={item} />)
          }
