...
export default class Detail extends Component<InspectDetailProps, InspectDetailState> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      flag: get(this.$router, 'params.from', '') !== 'examine'
    }
  }
  ...
  componentDidMount() {
    ...
    (async () => {
      try {
        ...
        //获取事件详情
        const detailResp = await getInspectDetail(inspectIdNumber, this.state.flag);
        this.setState({
        ...
        const inspectId = get(detailResp, 'data.id')
        //查看事件
        view(inspectId);

        //获取查看列表
        getViewList(inspectId).then(viewListResp => {
          this.setState({
            viewPageParams: {
  ...
  componentDidShow() {
    ...
    if (refresh) {
      ...
      //刷新事件详情数据
      getInspectDetail(inspectId, this.state.flag).then(detailResp => {
        this.setState({
          inspectDetail: detailResp.data,
        }, () => {
          Taro.hideLoading();
          this.getNewReplyList(get(detailResp, 'data.id'));
        });
      }).catch(() => {
        Taro.hideLoading();
      });
  ...
  getNewBehaviorList(tabId: number) {
    const { inspectDetail, inspectId: id } = this.state;
    const inspectId = get(inspectDetail, 'id', id)
  ...
  render() {
    ...
    const hasSupervisePermission: boolean = isAnyRole(userDetails.roles, [SUPERVISOR, ADMINISTRATOR, LEADER]);
