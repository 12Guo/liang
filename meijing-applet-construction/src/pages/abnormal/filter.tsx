...
export default class Filter extends Component<FilterProps, FilterState> {
    ...
    constructor(props) {
        ...
        const workSearchFilter = Taro.getStorageSync('abnormal-search-filter')
        this.state = {
            configList: [
                {
                    name: 'types', title: '事件类型', items: [
                        { label: '视频事件', value: 'SENTRY' },
    ...
    componentDidMount() {
        setTimeout(this.initDepartmentSelect, 500)
    }

    initDepartmentSelect = () => {
        const workSearchFilter = Taro.getStorageSync('abnormal-search-filter')
        if (isEmpty(workSearchFilter)) {
            const { userStore: { userDetails: { departmentInfo } } } = this.props
            const { configList, configChecked } = this.state
            const departments = get(configList, '[2].items', []) || []
            if (departments.find(department => department.value == departmentInfo.id)) {
                configChecked['departmentIds'] = departmentInfo.id.toString()
                this.setState({ configChecked })
            }
        }
    }
    ...
    saveBack = () => {
        ...
        Taro.setStorageSync('abnormal-search-filter', { ...configChecked, startTime, endTime })
        Taro.navigateBack()
    }
    ...
    onReset = () => {
        Taro.setStorageSync('abnormal-search-filter', {})
        const { startTime, endTime } = this.$router.params
        this.setState({
